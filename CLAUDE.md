...
All subagent executions are automatically logged to `.agent-logs/` in JSONL format (one JSON object per line).
...
.agent-logs/
├── TASK-XXX/
│   ├── plan-agents-[timestamp].jsonl
│   ├── build-agents-[timestamp].jsonl
│   ├── creative-agents-[timestamp].jsonl
│   └── [...]
└── c4-agents-[timestamp].jsonl
```
...
```json
{
  ...
  "agent_type": "build-coding-agent",
  "phase": "BUILD",
  "task_id": "TASK-001",
  "status": "started|in_progress|completed|error",
  "message": "Agent output or status message",
  "duration_ms": 5000,
  "error": null
}
```
...
- After a workflow command fails — look for `status: "error"` entries
- To identify bottlenecks — check `duration_ms` for agents that took unexpectedly long
- To debug agent output — search by `agent_type` or `task_id` to find specific executions
- To audit execution history — all logs are cumulative, creating a complete execution trail

**Cleanup:**

Agent logs accumulate over time. Remove old logs periodically:

```bash
# Remove logs older than 30 days
find .agent-logs -name "*.jsonl" -mtime +30 -delete

# Remove logs for a completed task
rm -rf .agent-logs/TASK-001/
```

See `.agent-logs/README.md` for detailed log querying and debugging instructions.
